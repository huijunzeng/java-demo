...
/**
 * @author zjh
 * @Description JDK1.8日期时间工具类
 * @date 2020/10/15 11:11
 * <p>
 * Instant类用于获取时间戳
 * LocalTime 时间类（不能操作日期）
 * LocalDate 日期类（不能操作时分秒）
 * LocalDateTime 日期时间类
 * LocalDate转化为具有时分秒的时间格式时，会自动补全为格林威治时间的时间（00：00：00）
...
public class DateUtil {
    ...
    public static Date instantToDateConverter(Instant instant) {
        Assert.notNull(instant, "instant can not be null");
    ...
    public static int getDayOfWeek(Date date) {
        Optional.ofNullable(date).orElseThrow(() -> new RuntimeException("date can not be null"));
        return getDayOfWeek(dateToLocalDateTimeConverter(date));
    }

    /**
     * 从LocalDateTime获得是当周的第几天 星期一为第一天，依次递加
     *
     * @param localDateTime
     * @return
     */
    ...
    /**
     * 从LocalDate获得是当周的第几天 星期一为第一天，依次递加
     *
     * @param localDate
     * @return
     */
    ...
    public static void main(String[] args) {
        ...
        System.out.println(localTimeToLocalDateTimeConverter(localTime));
        System.out.println(format(localDateTime, YYYY_MM_DD_HH_MM_SS_SSS));
        System.out.println(LocalDateTime.now(ZONE));
        System.out.println(new Date());
        System.out.println(localDateToDateConverter(localDate));
    }
}
